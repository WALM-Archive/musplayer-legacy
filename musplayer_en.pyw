...
def equo():
    ...
    def bits_sel(event):
        ...
        lbl_bits['text'] = f'bits {select_bits}'

    def sc_buf(buffer):
        global int_buffer
        buffer = float(buffer)
        int_buffer = round(buffer)
        lbl_buffer['text'] = f'buffer {int_buffer}'
    ...
    def sc_fr(freq):
        ...
        if freq == 44100:
            freq = float(freq)
            int_freq = round(freq)
            lbl_frequency['text'] = f'frequency {int_freq}'
        else:
            freq = float(freq)
            int_freq = round(freq)
            lbl_frequency['text'] = f'frequency {int_freq}'
    ...
    equodes.title('Equolizer')

    canequo = Canvas(equodes, width = 500, height = 300)
    canequo.pack()

    lbl_frequency = Label(canequo, text = f'frequency {freq} ', font = ('Nunito', 13))
    lbl_frequency.place(x = 20, y = 30)

    lbl_buffer = Label(canequo, text = f'buffer {buffer} ', font = ('Nunito', 13))
    lbl_buffer.place(x = 180, y = 30)

    lbl_bits = Label(canequo, text = f'bits {bits[1]} ', font = ('Nunito', 13))
...
def fun_mus_downloader():
    ...
    def downget():
        ...
        if format_combo.get() == '':
            messagebox.showerror('Download', 'Error in url \n or name')
        else:
            print('Get stupid')
        ...
        if directentry.get() == '':
            if os.path.isdir(directentry.get()):
                ...
                messagebox.showerror('Download', 'Error in directory')
        else:
            print('Get stupid')
        if urlcombo.get() == 'URL':   
            wget.download(varurl, f'{directentry.get()}{varname}')
            messagebox.showinfo('Download', 'download is\nDone')
        elif urlcombo.get() == 'Youtube URL':
            downdes.title('Please wait')
            downdes.update()
            yt = YouTube(f'{varurl}')
            ytnm = yt.title
            ...
            messagebox.showerror('Download', 'Error in change\n type url')

    downcan = Canvas(downdes, width=250, height=300)
    downcan.pack()
    varnameentry = StringVar()
    lblnameentry = Label(downcan, text = 'Music Name               format', font = ('Nunito', 13))
    ...
    typeurl = Label(downcan, text = 'Type URL', font = ('Nunito', 10))
    typeurl.place(x = 85, y = 150)
    directlabel = Label(downcan, text = 'directory for save', font = ('Nunito', 10))
...
def song_duration_time():
     ...
     if mus_name != '':
        try:
            ...
            messagebox.showerror('Musplayer', 'file damaged')
     else:
        pass
     ...
     if update_update == False:
        ...
        dl_mus.config(text=f'{str(converted_time)} - repetition')
...
def load_mus():
    ...
    global var_btn_mus_play_pause
    global playlist_move_remove_panel
    mus_name = filedialog.askopenfilename(title = "Select your cool music track",filetypes = (("mp3 Music Files","*.mp3"),("ogg Music Files","*.ogg"), ("wav Music Files", "*.wav")))
    print(f'loaded {mus_name}')
    pygame.mixer.music.stop()
    if mus_name == '':
        mus_label.config(text = 'Not found')
    else:
...
def mus_update():
    ...
    if update_update == False:
        ...
        update_update = True
        mus_loop = True
        messagebox.showinfo('repetition','repetition\non')
    elif update_update == True:
        pygame.mixer.music.stop()
        c_mus_stop()
        update_update = False
            ...
            if update_update == True:
             if playlist_move_remove_panel == int(1):
              ...
              try:
                ...
                messagebox.showerror('Musplayer', 'Error in repeat music\nfile damaged')
             ...
             if playlist_move_remove_panel == int(1):
              ...
              try:
                ...
                messagebox.showerror('Musplayer', 'Error in music\nfile damaged')
              des.title(name_musplay_playlist)
              lbl_name_playlist()
             elif playlist_move_remove_panel == int(0):
              des.title(name_mus)
...
def fun_prog_about():
    ...
    if theme == 'sun':
        ...
        ablbl = Label(abcan, text = f'musplayer\nver {upbut}\nMade by WebMast', font=('Arial', 20), fg = '#000', bg='#FFF')
        ...
        ablbl = Label(abcan, text = f'musplayer\nver {upbut}\nMade by WebMast', font=('Arial', 20), fg = '#FFF', bg='black')
...
def on_closing():
    ...
    def tray_mus_update():
        ...
        if update_update == False:
            ...
            if notify_support == True:
                icon_tray.notify('repeat on', 'musplayer')
            elif notify_support == False:
                print('Not supported notify on platform')
                print('repetition on')
        elif update_update == True:
            pygame.mixer.music.stop()
            playning = 'get_play'
            ...
            notify_support = icon_tray.HAS_NOTIFICATION
            if notify_support == True:
                icon_tray.notify('repeat off', 'musplayer')
            elif notify_support == False:
                print('Not supported notify on platform')
                print('repetition off')

    def mustray_version():
        global upbut
        icon_tray.notify(f'version {upbut}', 'musplayer')
    ...
    def mustray_play_on_pause():
        ...
        if playning == 'get_play':
            if mus_loop == True:
                if update_update == True:
                 ...
                 try:
                  ...
                  notify_support = icon_tray.HAS_NOTIFICATION
                  if notify_support == True:
                     icon_tray.notify('play with repeat', 'musplayer')
                  elif notify_support == False:
                    print('Not supported notify on platform')
                    print('play with repeat')
                 except:
                  if notify_support == True:
                    icon_tray.notify('damaged audio file', 'musplayer')
                  elif notify_support == False:
                    print('Not supported notify on platform')
                    print('damaged audio file')
                elif update_update == False:
                 mus_loop = 2
                 try:
                  pygame.mixer.music.play()
                  notify_support = icon_tray.HAS_NOTIFICATION
                  if notify_support == True:
                     icon_tray.notify('play ', 'musplayer')
                  elif notify_support == False:
                     print('Not supported notify on platform')
                     print('play ')
                 except:
            ...
            notify_support = icon_tray.HAS_NOTIFICATION
            if notify_support == True:
                icon_tray.notify('pause ', 'musplayer')
            elif notify_support == False:
                print('Not supported notify on platform')
                print('pause')
        elif playning == 'pause':
            ...
            notify_support = icon_tray.HAS_NOTIFICATION
            if notify_support == True:
                icon_tray.notify('unpause ', 'musplayer')
            elif notify_support == False:
                print('Not supported notify on platform')
                print('unpause')
...
can.create_line(0, 240, 600, 240, fill = 'black')
#время музыки
dl_mus = Label(can, text = "it's music time", bg='white', font = ('Nunito', 14), fg = 'black')
...
#лабель о плейлисте
lbl_playlist = Label(text='Playlist', bg = 'white', font = ('Nunito', 10))
